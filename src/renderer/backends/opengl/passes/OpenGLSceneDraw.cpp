...
void drawOpenGLSceneItem(OpenGLPassContext& context,
                         const RenderItem& item,
                         const glm::mat4& model,
                         const glm::vec4* overrideColor) {
    const CachedOpenGLMesh* cached =
        context.meshCache.resolve(item.meshAsset, context.frameNumber);
    if (cached == nullptr) {
        return;
    }
    ...
    for (const CachedOpenGLPrimitive& primitive : cached->primitives) {
        const MaterialAsset* material = resolveMaterial(
            context.registry, item.materialAsset, primitive.defaultMaterial);
        ...
        const auto resolveTextureBinding =
            ...
            const OpenGLTexture* texture = context.textureCache.resolve(
                candidate->texture, context.frameNumber);
            if (texture == nullptr) {
                return nullptr;
            }
            texture->bind(unit);
